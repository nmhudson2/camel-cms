<div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
    <form method="post" action='{{route( "actions/set-app-name" )}}'
        class="md:grid md:grid-cols-2 md:gap-6 mt-10 sm:mt-0">
        @csrf
    ...
    <x-section-border />
    <form method="post" action='{{route( "actions/set-app-root" )}}'
        class="md:grid md:grid-cols-2 md:gap-6 mt-10 sm:mt-0">
        @csrf
        <div class="md:col-span-1 flex justify-between">
            <div class="px-4 sm:px-0">
                ...
                <p name="description" class="mt-1 text-sm text-gray-600">Change Where Your App Redirects on 404</p>
            </div>
        </div>
        <div name="form">
            <div class="px-4 py-5 bg-white sm:p-6 shadow sm:rounded-tl-md sm:rounded-tr-md">
                <div class="col-span-6 sm:col-span-4 ">
                    <x-label for="root">Default Page</x-label>
                    <x-input type="text" value="{{env( 'DEFAULT_PAGE' )}}" name="root" />
                </div>
            </div>
            <div
                class="flex items-center justify-end px-4 py-3 bg-gray-50 text-end sm:px-6 shadow sm:rounded-bl-md sm:rounded-br-md">
    ...
    <x-section-border />
    <form method="post" action='{{route( "actions/set-active-theme" )}}' id='theme-form'
        class="md:grid md:grid-cols-2 md:gap-6 mt-10 sm:mt-0">
        @csrf
        <div class="md:col-span-1 flex justify-between">
            <div class="px-4 sm:px-0">
    ...
    <x-section-border />
    <form method="post" enctype="multipart/form-data" action='{{route( "actions/set-site-logo" )}}'
        class="md:grid md:grid-cols-2 md:gap-6 mt-10 sm:mt-0">
        @csrf
        <div class="md:col-span-1 flex justify-between">
            <div class="px-4 sm:px-0">
        ...
        <div name="form">
            <div class="px-4 py-5 bg-white sm:p-6 shadow sm:rounded-tl-md sm:rounded-tr-md">
                <div class="col-span-6 sm:col-span-4 ">
                    <x-label class='flex flex-row m-5' for='file_upload'>Current: <img class="w-10 ml-3"
                            src="{{asset( env( 'SITE_LOGO_PATH' ) )}}" alt="No Logo Selected"></x-label>
    ...
    <x-section-border />
    <form method="post" action='{{route( "actions/create-new-user" )}}'
        class="md:grid md:grid-cols-2 md:gap-6 mt-10 sm:mt-0">
        @csrf
        <div class="md:col-span-1 flex justify-between">
            <div class="px-4 sm:px-0">
